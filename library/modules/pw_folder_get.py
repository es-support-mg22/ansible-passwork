...
def _password_folder_get(
    ...
    with get_pw_session(api_server, token, master_key) as session:
        if (folder_id := folder_args.pop('folder_id', None)) is not None:
            return get_folder_by_id(session, api_server, folder_id)

        name: str = get_field(folder_args, 'name', ', если не указан folder_id!')
        vault: str = get_field(folder_args, 'vault', ', если не указан folder_id!')

        vault_id = get_vault(session, api_server, vault)['id']
        return get_folder(session, api_server, name, vault_id)
...
def main():
    ...
    result['response'] = _password_folder_get(api_server, token, master_key, folder_args)
    module.exit_json(**result)
